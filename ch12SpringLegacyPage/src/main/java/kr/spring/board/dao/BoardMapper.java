...
public interface BoardMapper {
	//부모글
	@Insert("INSERT INTO slpboard (board_num,title,"
		 + "content,filename,ip,mem_num) VALUES ("
		 + "slpboard_seq.nextval,#{title},#{content},"
		 + "#{filename,jdbcType=VARCHAR},#{ip},#{mem_num})")
	...
	@Select("SELECT * FROM slpboard JOIN slpmember "
		 + "USING(mem_num) WHERE board_num=#{board_num}")
	public BoardVO selectBoard(Long board_num);
	@Update("UPDATE slpboard SET hit=hit+1 "
	...
	//부모글 좋아요
	@Select("SELECT * FROM slpboard_fav "
		+ "WHERE board_num=#{board_num} AND mem_num=#{mem_num}")
	public BoardFavVO selectFav(BoardFavVO fav);
	@Select("SELECT COUNT(*) from slpboard_fav "
		+ "WHERE board_num=#{board_num}")
	public Integer selectFavCount(Long board_num);
	@Insert("INSERT INTO slpboard_fav (board_num,"
		+ "mem_num) VALUES (#{board_num},#{mem_num})")
	public void insertFav(BoardFavVO fav);
	@Delete("DELETE FROM slpboard_fav "
		+ "WHERE board_num=#{board_num} AND mem_num=#{mem_num}")
	...
	@Insert("INSERT INTO slpboard_reply (re_num,re_content,"
		  + "re_ip,board_num,mem_num) VALUES ("
		  + "slpreply_seq.nextval,#{re_content},#{re_ip},"
		  + "#{board_num},#{mem_num})")
	public void insertReply(BoardReplyVO boardReply);
	@Update("UPDATE slpboard_reply "
		  + "SET re_content=#{re_content},re_ip=#{re_ip},"
