...
public interface MemberMapper {
	...
	@Insert("INSERT INTO slpmember_detail "
		  + "(mem_num,name,passwd,phone,email,zipcode,"
	...
	//아이디 중복 및 로그인 체크
	@Select("SELECT mem_num,id,auth,passwd,nick_name,photo "
		  + "FROM slpmember LEFT OUTER JOIN slpmember_detail "
		  + "USING(mem_num) WHERE id=#{id}")
	...
	@Update("UPDATE slpmember_detail SET name=#{name},"
		  + "phone=#{phone},email=#{email},zipcode=#{zipcode},"
		  + "address1=#{address1},address2=#{address2},"
		  + "modify_date=SYSDATE WHERE mem_num=#{mem_num}")
	...
	public List<MemberVO> selectList(
			                   Map<String,Object> map);
	//회원권한 수정
	@Update("UPDATE slpmember SET auth=#{auth} "
	...
	@Update("UPDATE slpmember_detail SET name=#{name},"
		  ...
		  + "address2=#{address2} WHERE mem_num=#{mem_num}")
