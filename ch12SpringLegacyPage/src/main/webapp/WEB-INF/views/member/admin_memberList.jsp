		...
		<tr>
			<td>
				<c:if test="${member.auth==0}">
					${member.id}<c:if test="${!empty member.nick_name}">(${member.nick_name})</c:if>
				</c:if>
				<c:if test="${member.auth>0}">
					<a href="admin_update.do?mem_num=${member.mem_num}">${member.id}<c:if test="${!empty member.nick_name}">(${member.nick_name})</c:if></a>
				</c:if>
			</td>
			<td>${member.name}</td>
			<td>${member.phone}</td>
