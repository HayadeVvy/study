...
create table slpmember_detail(
 ...
 constraint slpmember_detail_fk foreign key (mem_num)
...
create table slpboard(
 ...
 constraint slpboard_fk foreign key (mem_num)
                   references slpmember (mem_num)
);
...
create table slpboard_fav(
 board_num number not null,
 mem_num number not null,
 constraint fav_slpboard_fk1 foreign key (board_num)
                       references slpboard (board_num),
 constraint fav_slpmember_fk2 foreign key (mem_num)
                       references slpmember (mem_num)
);
...
create table slpboard_reply(
 ...
 constraint reply_fk1 foreign key (board_num)
                       references slpboard (board_num),
 constraint reply_fk2 foreign key (mem_num)
                       references slpmember (mem_num)
);
