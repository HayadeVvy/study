...
public class ShopDAO {
	...
	public void selectOrders() {
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT order_num,cust_id,"
				+ "order_date,item_name,item_price "
	...
	//사용자 상품 구매
	public void insertOrder(String cust_id,
			                int item_num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "INSERT INTO sorder (order_num,"
				+ "cust_id,item_num) VALUES ("
				+ "sorder_seq.nextval,?,?)";
			//JDBC 수행 3단계
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	public void selectOrdersById(String cust_id) {
		...
		try {
			...
			sql = "SELECT order_num,order_date,"
				+ "item_name,item_price FROM "
				+ "sorder JOIN sitem "
				+ "USING(item_num) WHERE cust_id=? "
				+ "ORDER BY order_num DESC";
			//JDBC 수행 3단계
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
