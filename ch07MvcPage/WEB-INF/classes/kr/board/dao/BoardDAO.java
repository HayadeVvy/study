...
public class BoardDAO {
	...
	public void insertBoard(BoardVO board)
		...
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO zboard (board_num,title,content,
					                    filename,ip,mem_num)
	...
	public List<BoardVO> getListBoard(int start,int end,
		...
		try {
			...
			pstmt.setInt(++cnt, start - 1);//건너뛸 개수
			pstmt.setInt(++cnt, end - start + 1);//읽어올 개수
			//SQL문 실행
			rs = pstmt.executeQuery();
	...
	//부모글 상세
	public BoardVO getBoard(long board_num)
			throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			if(rs.next()) {
				...
				//로그인한 회원 번호와 조건 체크를 해야 하기 때문에 mem_num이 필요함
				board.setMem_num(rs.getLong("mem_num"));
				board.setId(rs.getString("id"));
				//채팅 허용 여부 체크
				board.setAuth(rs.getInt("auth"));
	...
	public void deleteBoard(long board_num)
		...
		try {
			...
			//채팅 메시지 삭제
			sql = """
					DELETE FROM zboard_chat
					WHERE chatroom_num 
					IN (SELECT chatroom_num 
					    FROM zboard_chatroom 
					    WHERE board_num=?)
					""";
			...
			//채팅방 삭제
			sql = 
	"DELETE FROM zboard_chatroom WHERE board_num=?";
	...
	//회원 번호와 게시물 번호를 이용한 좋아요 정보(회원이 게시물을 호출했을 때 좋아요 선택 여부 표시)
	...
	//좋아요 삭제
	public void deleteFav(BoardFavVO favVO)
			throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
	...
	//내가 선택한 좋아요 개수
	public int getBoardFavCount(long mem_num)
			throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT COUNT(*) FROM zboard_fav WHERE mem_num=?";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	//댓글 등록
	public void insertReplyBoard(BoardReplyVO vo)
			throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO zboard_reply (re_num,re_content,
					                  re_ip,mem_num,board_num)
	...
	public List<BoardReplyVO> getListReplyBoard(
		...
		try {
			...
			//?에 데이터 바인딩
			pstmt.setLong(1, board_num);
			pstmt.setInt(2, start - 1);//건너뛸 개수
			pstmt.setInt(3, end - start + 1);//읽어올 개수
			//SQL문 작성
			rs = pstmt.executeQuery();
	...
	//댓글 삭제
	public void deleteReplyBoard(long re_num)
			throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
	...
	//채팅 - 내가 쓴 게시글 총레코드수(검색 레코드수)
	public int getBoardCountByUser(String keyfield,
			                       String keyword,
			                       long mem_num)
	                                throws Exception{
		Connection conn = null;
	...
	public List<BoardVO> getListBoardByUser(
			              int start, int end,
			     String keyfield,String keyword,
			                  long mem_num)
	                         throws Exception{
		...
		try {
			...
			sql = """
					SELECT *
					FROM zboard 
					LEFT OUTER JOIN (SELECT board_num,MAX(chat_num) chat_num
					                 FROM zboard_chatroom 
					                 JOIN zboard_chat 
					                 USING (chatroom_num)
					                 GROUP BY board_num) USING(board_num)
					LEFT OUTER JOIN (SELECT board_num, 
					                        SUM(read_check) unread_cnt
					                 FROM zboard_chatroom 
					                 JOIN zboard_chat 
					                 USING(chatroom_num)
					                 WHERE mem_num!=?
				...
				+ sub_sql + 
				"""
					ORDER BY unread_cnt DESC NULLS LAST
					OFFSET ? ROWS FETCH NEXT ? ROWS ONLY		
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			...
			while(rs.next()) {
				...
				board.setReg_date(rs.getDate("reg_date"));
				//채팅 번호
				board.setChat_num(rs.getLong("chat_num"));
				//읽지 않은 채팅수
	...
	//채팅 - 채팅방 리스트
	public List<BoardChatVO> getChatList(
			            int start,int end, 
			            String keyfield,String keyword,
			            long board_num, long mem_num)
			            		throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql = """
					SELECT *
					FROM zboard_chat 
					JOIN zboard_chatroom c USING(chatroom_num)
					JOIN (SELECT MAX(chat_num) chat_num
					      FROM zboard_chat 
					      GROUP BY chatroom_num) USING(chat_num)
			...
			pstmt.setInt(++cnt, start -1);//건더뛸 개수
			pstmt.setInt(++cnt, end - start + 1);//읽어올 개수
			//SQL문 실행
			rs = pstmt.executeQuery();
	...
	//채팅 - 내가 채팅한 게시글 총레코드수(검색 레코드수)
	public int getBoardCountForMyChat(
			            String keyfield,String keyword,
			            long mem_num)
	                     throws Exception{
		Connection conn = null;
	...
	public List<BoardVO> getListBoardForMyChat(
			             int start, int end,
			             String keyfield, String keyword,
			             long mem_num)
	                                 throws Exception{
		...
		try {
			...
			pstmt.setInt(++cnt, start - 1);//건너뛸 개수
			pstmt.setInt(++cnt, end - start + 1);//읽어올 갯수
	...
	//채팅 - 채팅방 생성 여부 체크
	public BoardChatRoomVO checkChatRoom(
			long board_num, 
			long reader_num)
					throws Exception{
		Connection conn = null;
		...
		try {
			...
			sql = """
					SELECT 
					  c.*, 
					  (SELECT id FROM zmember 
					             WHERE mem_num=writer_num) writer_id,
					  (SELECT id FROM zmember
					             WHERE mem_num=reader_num) reader_id,
					   z.title                      
					FROM zboard_chatroom c 
	...
	//채팅 - 채팅방 정보 반환
	public BoardChatRoomVO selectChatRoom(long chatroom_num)
					                      throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	//채팅 - 채팅방 생성
	public void insertChatRoom(BoardChatRoomVO vo)
	                            throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO zboard_chatroom (chatroom_num,
							board_num,writer_num,reader_num)
	...
	//채팅 - 채팅 등록
	public void insertChat(BoardChatVO chat)
	                                 throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = null;
		...
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO zboard_chat (chat_num,
					      chatroom_num,mem_num,message)
	...
	//채팅 - 채팅 메시지 읽기(목록)
	public List<BoardChatVO> getChatDetail(
			           long user_num,long chatroom_num)
	                          throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		PreparedStatement pstmt2 = null;
		...
		try {
			...
			//내가 보낸 메시지가 아닌 상대방이 보낸 메시지를
			//읽었을 때 read_check를 1에서 0으로 변경
			sql = """
					UPDATE zboard_chat 
					SET read_check=0 WHERE mem_num!=? 
					AND chatroom_num=?
					""";
			pstmt = conn.prepareStatement(sql);
			...
			sql = """
					SELECT * 
					FROM zboard_chat JOIN zmember USING(mem_num)
