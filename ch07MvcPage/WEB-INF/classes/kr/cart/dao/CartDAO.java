...
public class CartDAO {
	...
	public void insertCart(CartVO cart)throws Exception{
		...
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO zcart (cart_num,item_num,
					                order_quantity,mem_num)
	...
	//회원 번호별 총구매 금액
	public int getTotalByMem_num(long mem_num)
	                               throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql = """
					...
					FROM (SELECT mem_num,
					             order_quantity * price AS sub_total 
					      FROM zcart JOIN zitem USING(item_num))
					WHERE mem_num=?
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
	...
	//장바구니 목록
	public List<CartVO> getListCart(long mem_num)
	                                throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
