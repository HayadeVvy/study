...
public class MemberDAO {
	...
	public void insertMember(MemberVO member)
		...
		try {
			...
			sql = """
					INSERT 
					INTO zmember_detail (mem_num,name,passwd,phone,
					                     email,zipcode,address1,address2)
					VALUES (?,?,?,?,?,?,?,?)
					""";
			pstmt3 = conn.prepareStatement(sql);
			//?에 데이터 바인딩
			pstmt3.setLong(++cnt, mem_num);
	...
	public MemberVO checkMember(String id)
		...
		try {
			...
			sql = """
					SELECT * FROM zmember 
					LEFT OUTER JOIN zmember_detail 
					USING(mem_num) 
					WHERE id=?
					""";
	...
	//회원 상세 정보
	public MemberVO getMember(long mem_num)
			                   throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql = """
					SELECT * FROM zmember 
					JOIN zmember_detail USING(mem_num)
					WHERE mem_num=?
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
	...
	public void updateMember(MemberVO member)
		...
		try {
			...
			//SQL문 작성
			sql = """
					UPDATE zmember_detail 
					SET name=?,phone=?,email=?,zipcode=?,
					    address1=?,address2=?,modify_date=SYSDATE
					WHERE mem_num=?					
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
	...
	//회원 등급 수정
	public void updateMemberByAdmin(int auth,
			                        long mem_num)
	                               throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
