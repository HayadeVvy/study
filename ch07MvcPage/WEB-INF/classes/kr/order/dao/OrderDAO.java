...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order,
		...
		try {
			...
			//주문정보 처리
			sql = """
					INSERT INTO zorder (order_num,order_total,
			...
			//주문상세정보 처리
			sql = """
					INSERT INTO zorder_detail (detail_num,item_num,
	...
	public int getOrderCount(String keyfield,
		...
		try {
			...
			//하나의 주문에서 상품이 여러개 존재할 수 있기 때문에
			//상품명을 하나의 행에 보여주기 위해 LISTAGG를 사용함.
			sql = """
					SELECT COUNT(*) FROM zorder 
					JOIN (SELECT order_num,LISTAGG(item_name,',')
	...
	//주문 삭제
	//관리자/사용자 - 주문 상세
	public OrderVO getOrder(long order_num)
			               throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public void updateOrder(OrderVO order)
		...
		try {
			...
			sql = """
					UPDATE zorder
					SET receive_name=?,receive_post=?,
					    receive_address1=?,receive_address2=?,
					    receive_phone=?,notice=?,
					    modify_date=SYSDATE
					WHERE order_num=?
					""";
			//PreparedStatement 객체 생성
			...
			pstmt.setLong(++cnt, order.getOrder_num());
			//SQL문 실행
			pstmt.executeUpdate();
		}catch(Exception e) {
			throw new Exception(e);
	...
	public void updateOrderCancel(long order_num)
		...
		try {
			...
			sql = """
					UPDATE zorder 
					SET status=5,modify_date=SYSDATE
				    WHERE order_num=?
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
