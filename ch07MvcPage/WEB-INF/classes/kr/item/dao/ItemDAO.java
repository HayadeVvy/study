...
public class ItemDAO {
	...
	public void insertItem(ItemVO item)throws Exception{
		...
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO zitem (item_num,name,price,
					                   quantity,photo1,photo2,
					                   detail,status)
	...
	public void updateItem(ItemVO item)throws Exception{
		...
		try {
			...
			sql = """
					UPDATE zitem 
					SET name=?,price=?,quantity=?,detail=?,
					    modify_date=SYSDATE,status=? 
					"""
					+sub_sql +
				"""
					WHERE item_num=?		
	...
	public List<ItemVO> getListItem(
		...
		try {
			...
			//status가 0이면 status > 0 처리 ,1(미표시),2(표시) 
			//                              모두 호출 -> 관리자용
			//status가 1이면 status > 1 처리 ,2(표시) 호출 -> 사용자용
			//SQL문 작성
			sql = """
					SELECT * FROM zitem
					WHERE status > ? 
