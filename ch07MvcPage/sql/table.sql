...
create table zboard_chatroom(
 ...
 reader_num number not null,--읽은 사람(채팅 희망자) 회원 번호
 room_date date default sysdate not null,
 constraint chatroom_pk primary key (chatroom_num),
 constraint chatroom_fk1 foreign key (board_num)
                      references zboard (board_num),
 constraint chatroom_fk2 foreign key (writer_num)
                      references zmember (mem_num),
 constraint chatroom_fk3 foreign key (reader_num)
                      references zmember (mem_num)
);
...
create table zboard_chat(
 ...
 chatroom_num number not null, 
 mem_num number not null, --채팅 메시지 작성자
 ...
 constraint chat_fk1 foreign key (chatroom_num)
            references zboard_chatroom (chatroom_num),
 constraint chat_fk2 foreign key (mem_num)
                         references zmember (mem_num)
);
...
create table zorder(
 ...
 status number(1) default 1 not null, -- 배송 상태(1:배송대기,2:배송준비중,3:배송중,4:배송완료,5:주문취소)
...
create table zorder_detail(
 ...
 constraint zorder_detail_fk1 foreign key (order_num)
