...
public class BoardDAO {
	...
	public void insertBoard(BoardVO boardVO)
		...
		try {
			...
			//SQL문 작성
			sql = """
					INSERT INTO sboard (board_num,title,content,ip,num)
	...
	public List<BoardVO> getBoardList(int startRow,
		...
		try {
			...
			sql = """
					SELECT * 
					FROM sboard
					JOIN smember USING (num)
					ORDER BY board_num DESC
					OFFSET ? ROWS FETCH NEXT ? ROWS ONLY
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, startRow - 1);//offset에 지정 : 건너뛰는 개수
			pstmt.setInt(2, endRow - startRow + 1);//fetch next에 지정 : 읽어올 개수
	...
	public BoardVO getBoard(long board_num)
		...
		try {
			...
			sql = """
					SELECT * 
					FROM sboard JOIN smember USING(num) 
					WHERE board_num=?
					""";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
